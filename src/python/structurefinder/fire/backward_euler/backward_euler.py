...
class GeomoptViaBackwardEulerFIRE(pp.ModuleBase):
    ...
    def run_(self, inputs, submods):
        ...
        def grad_func(new_coord, molecule):
            current_molecule = updated_molecule_coord(new_coord, molecule)
            current_points = current_molecule.nuclei.as_nuclei().charges.point_set.as_point_set() # The problem is here when using 0.0 coordinates
...
#-----------------------------------------------------------------------------------------------------------------
        def backwardeuler_FIRE(molecule, R_xyz, h0 = 0.03, alpha = 0.1, t_max=0.3, numbcycles=150, error_power = -8, time_step_update = 5):
            mol_coord = R_xyz
            Molecule = [molecule]
            #----------------------------------------------------------------------
            ...
            #----FORCE-----------------------------------------------------------
            F_0 =  -np.array(grad_func(mol_coord, molecule))      #<-- Initial Gradient
            ...
            while k < (Ncycle):
                ...
                #---- FIRE VELOCITY CORRECTION -------------------------------
                    VEL[i] = (1-alpha)*VEL[i] + alpha*np.linalg.norm(VEL[i])*force_unit
                #-------------------------------------------------------------
                ...
                #--- ERRORS ----------------------------------------------------------
                Rxyz_error = np.linalg.norm(coord_vector[i] - coord_vector[i+1])
                ...
                if (Rxyz_error < error) and  (egy_error < error) and (force_error < error):
                    break
                ...
                if k == Ncycle:
                    print(f"Maximum number of optimization steps achieved: {Ncycle}")
                    break
                i+=1
